...
import {
  ...
  SflScalarTester,
  SflTester,
} from "../interfaces/sfl.ts";

const RE_WHITESPACE = /^[ \t\r\v\f]+/;
const RE_NEWLINE = /^[\n]+/;

const RE_TESTERNAME = /^([a-zA-Z_]\w*)/;
const RE_TESTERPARAM =
  /^(?:(null|true|false)|("(?:[^"\\]*|\\.)*")|('(?:[^'\\]*|\\.)*')|(-?\d+(?:\.\d*)?)|\/((?:[^/\\]*|\\.)+)\/(igm|img|gim|gmi|mig|mgi|ig|im|gi|gm|mi|mg|i|g|m)?)?/;
    ...
    if (buf[0] === "(") {
      ...
      while (match) {
        if (match[1]) {
          ...
          params.push(
            new RegExp(match[5].replace(/\\\//g, "/"), match[6] || undefined),
          );
        } else {
