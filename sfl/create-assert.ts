...
import { InvalidValueError } from "../errors/invalid-value-error.ts";
...
function error(name: string, args: any[] = []): string {
...
function tester(curr: SflTester, val: string): string {
  switch (curr.type) {
    case "object": {
      ...
      nxt += `if(typeof ${val}!=="object"||${val}===null){return [${
        error("object")
      }]}`;
    ...
    case "array": {
      ...
      if (hasMin && hasMax && curr.min === curr.max) {
        nxt += `if(${val}.length!==${curr.max}){return[${
          error("array_length", [curr.min])
        }]}`;
      } else if (hasMin && hasMax) {
        nxt +=
          `if(${val}.length<${curr.min}||${val}.length>${curr.max}){return[${
            error("array_length_between", [curr.min, curr.max])
          }]}`;
      } else if (hasMin) {
        nxt += `if(${val}.length<${curr.min}){return[${
          error("array_length_min", [curr.min])
        }]}`;
      } else if (hasMax) {
        nxt += `if(${val}.length>${curr.max}){return[${
          error("array_length_max", [curr.max])
        }]}`;
      }
      const merged = uid();
      const i = uid();
    ...
    case "and": {
      ...
      return "(function(){" +
        `var ${result}=${
          curr.params.map((param) => tester(param, val)).join(
            ";" +
...
export function createAssert(
  ...
  return new Function(
    ...
    let message =
      (messages[reason] ||
        ["The :path is invalid value.", "It is invalid value."])[path ? 0 : 1];
    message = message.replace(":path", path);
    params.forEach((param, index) => {
      message = message.replace(`:param${index}`, JSON.stringify(param));
    });
    return message.replace(/:param\d+/g, "").replace(
      /:params/g,
